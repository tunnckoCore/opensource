...
import { parseHeader } from '../../src/header.ts';

test('.parseHeader throw if not a string given', () => {
  // @ts-expect-error yeah
  expect(() => parseHeader(123)).toThrow(TypeError);
  // @ts-expect-error yeah
  expect(() => parseHeader(123)).toThrow(/expect `header` to be non empty string/);
  expect(() => parseHeader('')).toThrow(/expect `header` to be non empty string/);
});

test('.parseHeader throw when invalid conventional commits', () => {
  expect(() => parseHeader('fix bar qux')).toThrow(Error);
  expect(() => parseHeader('fix bar qux')).toThrow(/<type>\[optional scope]: <description>/);
});
